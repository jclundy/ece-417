...
Product = zeros([size(image), length(p)]);

for k=1:length(p)
    Rad = R_pq(p(k), q(k), R);    % get the radial polynomial
    Product(:, :, k) = conv2(double(image(x,y)),Rad.*exp(q(k)*1i.*Theta), 'same');
    Product(:, :, k) = Product(:, :, k)*(p(k)+1)/pi;
    Product(:, :, k) = abs(Product(:, :, k));
end
...
newImage = weightNormalized(image, Product, degreeOfFiltering);
